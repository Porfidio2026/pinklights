...
async function hash(order: string, amount: string, cur: string, desc: string, pass = PASS) {
...
Deno.serve(async (req) => {
  try {
    // Proxies calls with merchant credentials; must not be open to every user.
    ...
    if (role !== 'service_role') {
      return Response.json({ error: 'Service role required' }, { status: 403 });
    }
    ...
    const api = body.apiUrl || API;
    const key = body.merchantKey || KEY;
    const pass = body.merchantPassword || PASS;

    for (const name of names) {
      const order = `PLTEST-${Date.now()}-${Math.random().toString(36).slice(2, 8).toUpperCase()}`;
      const h = await hash(order, amount, 'USD', desc, pass);
    ...
    return Response.json({
      ...
      siteUrl: SITE_URL,
      usingOverrides: Boolean(body.merchantKey || body.merchantPassword),
      results,
    });
  } catch (err) {
