...
import { COMPARE_HUB_TRANSLATIONS } from '@/data/seo/pages';
import { Scale, ArrowRight } from 'lucide-react';

const LocaleCompareHub = () => {
  ...
  const hub = COMPARE_HUB_TRANSLATIONS[locale];
  const ui = UI_STRINGS[locale];
  const breadcrumbs = [{ name: ui.compare, url: `/${locale}/compare` }];
    ...
    <div className="min-h-screen bg-background py-8 px-4">
      ...
      <div className="max-w-3xl mx-auto">
        ...
        <div className="grid sm:grid-cols-2 gap-4 mb-8">
          {hub.comparisons.map((comp) => (
            <Link key={comp.slug} to={`/${locale}/compare/${comp.slug}`} className="group">
...
export default LocaleCompareHub;
